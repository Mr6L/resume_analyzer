@echo off
chcp 65001 > nul
echo 启动简历分析系统
echo.

echo 检查后端服务状态...
curl -s http://127.0.0.1:5000/health > nul
if errorlevel 1 (
    echo 错误: 后端服务未运行，请先启动后端服务
    echo 请在另一个命令行窗口运行: cd backend && python app.py
    pause
    exit
)
echo 后端服务正常

echo.
echo 启动前端界面...
echo 前端地址: http://127.0.0.1:7862
echo 按 Ctrl+C 停止服务
echo.

python -c "
import gradio as gr
import requests
import json

def analyze_resume(file):
    if file is None:
        return '请上传文件', '请上传文件', '请上传文件'

    try:
        files = {'file': (file.name, open(file.name, 'rb'))}
        response = requests.post('http://127.0.0.1:5000/full_analysis', files=files, timeout=120)
        files['file'][1].close()

        if response.status_code == 200:
            result = response.json()
            if result.get('success'):
                parsed = json.dumps(result.get('parsed_data', {}), ensure_ascii=False, indent=2)
                analysis = str(result.get('analysis', ''))
                recommendations = str(result.get('recommendations', ''))
                return parsed, analysis, recommendations
            else:
                error = result.get('error', '未知错误')
                return f'错误: {error}', '', ''
        else:
            return f'HTTP错误: {response.status_code}', '', ''
    except Exception as e:
        return f'异常: {str(e)}', '', ''

def check_backend():
    try:
        response = requests.get('http://127.0.0.1:5000/health', timeout=5)
        return '后端正常' if response.status_code == 200 else f'后端异常: {response.status_code}'
    except:
        return '无法连接后端'

with gr.Blocks(title='简历分析') as demo:
    gr.Markdown('# 简历分析系统')

    with gr.Row():
        with gr.Column():
            file_input = gr.File(label='上传简历(.docx)', file_types=['.docx'])
            analyze_btn = gr.Button('开始分析', variant='primary')
            status_btn = gr.Button('检查后端')
            status_text = gr.Textbox(label='状态', lines=1)

        with gr.Column():
            with gr.Tabs():
                with gr.TabItem('解析结果'):
                    parsed_output = gr.Textbox(label='解析', lines=15, max_lines=20)
                with gr.TabItem('AI分析'):
                    analysis_output = gr.Textbox(label='分析', lines=15, max_lines=20)
                with gr.TabItem('岗位推荐'):
                    recommendations_output = gr.Textbox(label='推荐', lines=15, max_lines=20)

    analyze_btn.click(analyze_resume, inputs=[file_input], outputs=[parsed_output, analysis_output, recommendations_output])
    status_btn.click(check_backend, outputs=[status_text])

demo.launch(server_name='127.0.0.1', server_port=7862, share=False, inbrowser=True)
"